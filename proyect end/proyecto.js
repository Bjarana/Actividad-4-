document.addEventListener('DOMContentLoaded', function() {
    ...
    async function cargarCategorias() {
        try {
            const response = await fetch('http://localhost:5500/categories/categories');
            if (!response.ok) {
    ...
    async function cargarProductos() {
        try {
            ...
            const response = await fetch('http://localhost:5500/products/products', {
                headers: {
            ...
            productos.forEach(producto => {
                ...
                deleteButton.addEventListener('click', async () => {
                    try {
                        const response = await fetch(`http://localhost:5500/products/products/${producto.id}`, {
                            method: 'DELETE',
                            headers: {
                                'Authorization': `Bearer ${token}`,
                            }
